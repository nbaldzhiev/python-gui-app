...
from views import MainView, MAX_SUBMISSIONS, MAX_DOCUMENTS
...
class Controller:
    ...
    def __init__(self):
        ...
        self.submissions = {str(i): {'meta': {}, 'documents': {str(j): {'content': '', 'meta': {}} for j in range(1, MAX_DOCUMENTS + 1)}} for i in range(1, MAX_SUBMISSIONS + 1)}
        #
        ...
        self.main_view.subs_ctrl.Bind(wx.EVT_SPINCTRL,
                                      self.manage_submissions)
        self.main_view.send_btn.Bind(wx.EVT_BUTTON,
                                     self.on_send_click)
        wx.Button.FindWindowByName('sub_1_btn').Bind(wx.EVT_BUTTON,
                                    self.show_sub_meta)
        wx.CheckBox.FindWindowByName('sub_1_dir').Bind(wx.EVT_CHECKBOX,
                                        self.on_dir_checkbox_click)
        wx.Frame.FindWindowByName(f'sub_1_view').save_button.Bind(
                            wx.EVT_BUTTON, self.on_sub_meta_save)
        wx.Frame.FindWindowByName(f'sub_1_view').Bind(wx.EVT_CLOSE,
                                            self.on_sub_meta_close)
        wx.SpinCtrl.FindWindowByName('sub_1_documents').Bind(wx.EVT_SPINCTRL,
    ...
    def on_send_click(self, event):
        ...
        for i in range(1, num_of_subs + 1):
            if not wx.Frame.FindWindowByName(f'sub_{str(i)}_view').\
                                                checkbox.IsChecked():
            ...
            if wx.CheckBox.FindWindowByName(f'sub_{str(i)}_dir').IsChecked():
                ...
                num_of_docs = \
                    wx.SpinCtrl.FindWindowByName(f'sub_{i}_documents').GetValue()
                ...
                for j in range(1, int(num_of_docs) + 1):
                    if submissions[str(i)]['documents'][str(j)]['content'] == '' and \
                        not wx.Frame.FindWindowByName(f"sub_{str(i)}_meta_{str(j)}_view").checkbox.IsChecked():
                        del submissions[str(i)]['documents'][str(j)]
                    elif submissions[str(i)]['documents'][str(j)]['content'] != '' and \
                        not wx.Frame.FindWindowByName(f"sub_{str(i)}_meta_{str(j)}_view").checkbox.IsChecked():
                        del submissions[str(i)]['documents'][str(j)]['meta']
                    elif submissions[str(i)]['documents'][str(j)]['content'] == '' and \
                        wx.Frame.FindWindowByName(f"sub_{str(i)}_meta_{str(j)}_view").checkbox.IsChecked():
                        del submissions[str(i)]['documents'][str(j)]['content']

                for j in range(int(num_of_docs) + 1, MAX_DOCUMENTS + 1):
                    del submissions[str(i)]['documents'][str(j)]

                if len(submissions[str(i)]['documents']) == 0 and \
                                    'meta' not in submissions[str(i)]:
                    submissions[str(i)] = {}
                elif len(submissions[str(i)]['documents']) == 0 and \
                                    'meta' in submissions[str(i)]:
                    del submissions[str(i)]['documents']

        self.model.upload_submissions(env_url=env_url, token=token,
    ...
    def bind_sub_meta(self) -> None:
        ...
        window."""
        for i in range(self.main_view.prev_num_of_subs + 1, self.main_view.num_of_subs + 1):
            wx.Button.FindWindowByName(f'sub_{i}_btn').\
                Bind(wx.EVT_BUTTON, self.show_sub_meta)
            wx.Frame.FindWindowByName(f'sub_{i}_view').save_button.\
                Bind(wx.EVT_BUTTON, self.on_sub_meta_save)
            wx.Frame.FindWindowByName(f'sub_{i}_view').\
                Bind(wx.EVT_CLOSE, self.on_sub_meta_close)
            wx.CheckBox.FindWindowByName(f'sub_{i}_dir').Bind(wx.EVT_CHECKBOX,
                                        self.on_dir_checkbox_click)
            wx.SpinCtrl.FindWindowByName(f'sub_{i}_documents').\
                Bind(wx.EVT_SPINCTRL, self.manage_documents)
    ...
    def on_dir_checkbox_click(self, event) -> None:
        ...
        id = re.search(r"sub_([0-9]+)_dir",
                       event.GetEventObject().GetName()).group(1)
        if event.GetEventObject().IsChecked():
            ...
            HyperlinkCtrl.FindWindowByName(f'sub_{id}_dir_link'). \
                            Bind(EVT_HYPERLINK, self.open_dir_dialog)
        else:
    ...
    def manage_documents(self, event) -> None:
        ...
        self.bind_doc_meta(num_of_docs=num_of_docs,
                                    id=id)
    ...
    def bind_doc_meta(self, num_of_docs: int, id: str) -> None:
        ...
        for i in range(current_number + 1, num_of_docs + 1):
            HyperlinkCtrl.FindWindowByName(f'sub_{id}_link_{i}').\
                Bind(EVT_HYPERLINK, self.open_file_dialog)
            wx.Button.FindWindowByName(f'sub_{id}_meta_{i}').\
                Bind(wx.EVT_BUTTON, self.show_doc_meta)
            wx.Frame.FindWindowByName(f"sub_{id}_meta_{i}_view").\
                save_button.Bind(wx.EVT_BUTTON, self.on_doc_meta_save)
            wx.Frame.FindWindowByName(f"sub_{id}_meta_{i}_view").\
                Bind(wx.EVT_CLOSE, self.on_doc_meta_close)

    def show_doc_meta(self, event) -> None:
        """Event handler for when a user clicks on a submission
        metadata button."""
        ids = re.search(r"sub_([0-9]+)_meta_([0-9]+)", event.GetEventObject().GetName()).groups()
        wx.Frame.FindWindowByName(f"sub_{ids[0]}_meta_{ids[1]}_view").Show()
        wx.Button.FindWindowByName(f'sub_{ids[0]}_meta_{ids[1]}').\
            SetForegroundColour(self.colour_db.Find('GREY'))
    ...
    def open_file_dialog(self, event) -> None:
        ...
        ids = re.search(r"sub_([0-9]+)_link_([0-9]+)", event.GetEventObject().GetName()).groups()
        HyperlinkCtrl.FindWindowByName(f'sub_{ids[0]}_link_{ids[1]}').\
            SetVisitedColour(self.colour_db.Find('GREY'))
        with wx.FileDialog(None, "Open Document File",
    ...
    def open_dir_dialog(self, event) -> None:
        ...
        HyperlinkCtrl.FindWindowByName(f'sub_{id}_dir_link'). \
                        SetVisitedColour(self.colour_db.Find('GREY'))
        with wx.DirDialog(None, "Open Documents Directory",
    ...
    def on_main_view_close(self, event) -> None:
        ...
        if wx.MessageBox("Do you really want to close the application?",
                         "Please confirm",
