...
class MainView(wx.Frame):
    ...
    def __init__(self, parent):
        ...
        self.url_ctrl = wx.TextCtrl(
            self, value=env_url, size=(300, -1), style=wx.TE_PROCESS_ENTER
        )
        self.token_text = wx.StaticText(self, label="Token:")
        self.token_ctrl = wx.TextCtrl(
            self, size=(300, -1), style=wx.TE_PROCESS_ENTER, value=token
        )
        ...
        self.sub_1_text = wx.StaticText(self, label="Submission 1:", name="sub_1_title")
        self.sub_1_meta = wx.Button(
            self, id=1, label="Metadata", style=wx.BU_EXACTFIT, name="sub_1_btn"
        )
        self.sub_1_dir = wx.CheckBox(
            self, label="Directory", style=wx.CHK_2STATE, name="sub_1_dir"
        )
        self.sub_1_dir.SetToolTip(
            tipString="Enable to allow opening (recursively) a directory with documents."
        )
        self.sub_1_documents = wx.SpinCtrl(
            self, value="0", min=0, max=MAX_DOCUMENTS, name="sub_1_documents"
        )
        self.sub_1_documents.SetToolTip(
        ...
        self.bs_elements: List[Tuple] = [
            (self.fgs, 1, wx.EXPAND | wx.ALL, 5),
            (self.fgs_subs, 0, wx.EXPAND | wx.ALL, 5),
        ]

        self.fgs.AddMany(self.fgs_elements)
        self.bs_subs_1.Add(
            wx.StaticLine(self, wx.LI_VERTICAL, name="self.static_line_1")
        )
        ...
        for i in range(2, MAX_SUBMISSIONS + 1):
            var_name = f"self.bs_subs_{i}"
            exec(
                f'self.bs_subs_{i}.Add(wx.StaticLine(self, wx.LI_VERTICAL, name="self.static_line_{i}"))'
            )
    ...
    def create_submissions(self) -> None:
        ...
        for i in range(self.prev_num_of_subs + 1, self.num_of_subs + 1):
            ...
            wx.Button(self, label="Metadata", style=wx.BU_EXACTFIT, name=f"sub_{i}_btn")
            wx.CheckBox(
                self, label="Directory", style=wx.CHK_2STATE, name=f"sub_{i}_dir"
            ).SetToolTip(
                tipString="Enable to allow opening (recursively) a directory with documents."
            )
            wx.SpinCtrl(
                self, value="0", min=0, max=MAX_DOCUMENTS, name=f"sub_{i}_documents"
            ).SetToolTip(tipString="Enter the number of documents to open")
            SubmissionMetadataView(None, f"sub_{i}_view")

            var_name = f"self.bs_subs_{i}"
            exec(
                var_name
                + ".Add(wx.StaticText.FindWindowByName(f'sub_{i}_title'), 0, wx.ALIGN_CENTER_HORIZONTAL)"
            )
            exec(
                var_name
                + ".Add(wx.Button.FindWindowByName(f'sub_{i}_btn'), 0, wx.ALIGN_CENTER_HORIZONTAL)"
            )
            exec(
                var_name
                + ".Add(wx.CheckBox.FindWindowByName(f'sub_{i}_dir'), 0, wx.ALIGN_CENTER_HORIZONTAL)"
            )
            exec(
                var_name
                + ".Add(wx.SpinCtrl.FindWindowByName(f'sub_{i}_documents'), 0, wx.ALIGN_CENTER_HORIZONTAL)"
            )
    ...
    def create_documents(self, num_of_docs: int, id: str) -> None:
        ...
        for i in range(current_number + 1, num_of_docs + 1):
            ...
            wx.Button(
                self, label="Metadata", style=wx.BU_EXACTFIT, name=f"sub_{id}_meta_{i}"
            )
            DocumentMetadataView(None, f"sub_{id}_meta_{i}_view")

            exec(
                f"self.bs_subs_{id}.Add(HyperlinkCtrl.FindWindowByName(f'sub_{id}_link_{i}'), 0, wx.ALIGN_CENTER_HORIZONTAL)"
            )
            exec(
                f"self.bs_subs_{id}.Add(wx.Button.FindWindowByName(f'sub_{id}_meta_{i}'), 0, wx.ALIGN_CENTER_HORIZONTAL)"
            )
    ...
    def create_dir(self, id: str) -> None:
        ...
        exec(
            f"self.bs_subs_{id}.Add(HyperlinkCtrl.FindWindowByName(f'sub_{id}_dir_link'), 0, wx.ALIGN_CENTER_HORIZONTAL)"
        )
